...
editorNeo = new $.fn.dataTable.Editor( {
		ajax: {url:"php/neo-db.php?lang="+languageW,type:"POST"},
		table: "#exampleNeo",
		lang:languageW
	} );
...
$(document).ready( function () {
				$('#exampleNeo').dataTable().columnFilter({
					aoColumns: [ 
						...
						{ sSelector: "#example_configp",type: "text", bRegex: true, bSmart: true }, // confi morph
						{ sSelector: "#example_configm",type: "text", bRegex: true, bSmart: true },// config morph detail
...
$('#exampleNeo tbody').on('click', 'td.details-control4', function () {
	...
	console.log(row.data());
 	if (editorNeo.lang == undefined){editorNeo.lang='fr';}
        if ( row.child.isShown() ) {
            ...
            // Open this row
            //console.log(row.data())
	    $.blockUI({ message: '<h1><img src="../img/wait.gif" /> Chargement en cours...</h1>' });
            get_neo_info_mysqldata(editorNeo.lang, row.data(), function(data) // from mysql
            {
        	    ...
        	    details.addClass('stat_res');
            	    $.unblockUI();
		    if (typeof data == 'number'){
            		//$('#neoResults'+ editorNeo.lang).modal();
...
$("#neoinfoBtnIt").on('click',function(){
	$("#neoinfoBtnIt").replaceWith('<a href="#" class="btn btn-info" id="neoinfoBtnIt"><i class="fa fa-circle-o-notch fa-spin"></i> Chargement des données (italien)...</a>');
...
//	    if (lang == 'Fr'){
	        ...
	        build_corpus_info_lang_cnt2(data,"Fr");
//	        }
//	    else{
