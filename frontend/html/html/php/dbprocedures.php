...
function save_to_neo2($lang,$user){
  $connect= mysqli_connect('localhost','root','neoveille','datatables'); 
	...
	if (!(mysqli_query($connect,"CALL insert_into_neo2('$lang','$user')"))){
	  ...
	  echo "Les néologismes validés ont été reversés dans la base de description générale. Utilisateur : " . $user;
	}
}
...
function save_to_dict_global2($lang,$user){
  ...
  $connect= mysqli_connect('localhost','root','neoveille','datatables'); 
	...
	if (!(mysqli_query($connect,"CALL insert_into_dico_termino_global2('$lang','$user')"))){
	  ...
	  echo "Les formes à exclure ont été enlevées de la base et stockées dans le dictionnaire d'exclusion. Utilisateur courant : " . $user;
	}
	}
	}
	}
...
function getcorpusinfo(){
  $res="";
  $conn= mysqli_connect('localhost','root','connect','rssdata'); 
