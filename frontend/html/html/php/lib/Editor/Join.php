...
class Join extends DataTables\Ext {
	...
	/**
	 * Specify the property that the data will be sorted by, or a custom sort
	 * function.
     *
	 * @param  string|callable $order Property to order the data by, or the
	 *   function
	 * @return Join Self for chaining
	 */
	...
	{
		$order = $this->_customOrder;

		if ( ! $order || ! $data ) {
			return;
		}
		else if ( is_string( $order ) ) {
			$clause = explode(' ', $order);
			$orderProp = $clause[0]; // always exists

			usort( $data, function ( $a, $b ) use ( $orderProp ) {
				return is_numeric($a[ $orderProp ]) && is_numeric($b[ $orderProp ]) ?
					($a[ $orderProp ]*1) - ($b[ $orderProp ]*1) :
					strcmp( $a[ $orderProp ], $b[ $orderProp ] );
			} );

			if ( count($clause) > 1 && strcasecmp($clause[1], 'desc') === 0 ) {
