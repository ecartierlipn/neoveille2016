...
## global config
lang_solr={'france':'rss_french','pologne':'RSS_polish','Rép. Tchèque':'RSS_czech','russie':'RSS_russian','chine':'RSS_chinese','grèce':'RSS_greek','brésil':'RSS_brasilian'}
...
hunspell['chine']='/opt/nlp_tools/dictionaries/zh_ZH/zh_ZH'
...
class document:
    ...
    def ling_analyze(self, ana_type):
        ...
        elif ana_type == 'hunspell_gen':
            self.hunspell_check_gen(self.contents)
        else:
            ...
            if result:
                ...
                for l2 in lines[1:-2]:
                    ...
                    if len(l)==0 or len(l)<3:
                        log.info("Empty or too short string : " + str(l))
                        continue
                    if l[0] in ('*','+') and re.search(r"-",s):
                        log.info("Compound word : " + l + "\n")
                        continue
                    if l[0] in ('*','+'):
                        ...
                        data = re.split(r" ",l)
                        print(data[1])                        
                        if data[1].islower() and not(data[1] in dict_exclusion.keys()):
                            print(" is lower")
                            unknown[data[1]]=1
                    elif l[0] == '&':
                        #print "Hunspell results : " + l2
                        ...
                        if (m):
                            ...
                            if dist<2:
                                ...
                                #print "Distance from : " + unicode(sugg[0]) + ":" + str(dist) 
                                print(m.group(1))
                                if m.group(1).islower()  and not(m.group(1) in dict_exclusion.keys()):
                                    print(" is lower")
                                    unknown[m.group(1)]=1                                
                        else:
                            continue
...
if __name__ == '__main__':
    ...
    logging.basicConfig(format=FORMAT, datefmt='%m/%d/%Y %I:%M:%S %p', filename="./log/detect_neologisms_gr.py.log", filemode='w', level=logging.INFO)    
