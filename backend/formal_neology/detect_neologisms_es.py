...
pos_ana={}
pos_ana['espagne']='/opt/nlp_tools/treetagger/cmd/tree-tagger-spanish ' # ou spanish-ancora
...
#pos_ana['italie']='/opt/nlp_tools/treetagger/cmd/tree-tagger-italian '
...
## global config
lang_solr={'espagne':'rss_spanish','italie':'rss_italian','france':'rss_french','pologne':'RSS_polish','Rép. Tchèque':'RSS_czech','russie':'RSS_russian','chine':'RSS_chinese','gr':'RSS_greek','brésil':'RSS_brasilian'}
# global variables : hunspell parameters
hunspell={}
hunspell['espagne']='/opt/nlp_tools/dictionaries/es/es_ANY'
...
#hunspell['italie']='/opt/nlp_tools/dictionaries/it_IT/it_IT'
iso={}
iso['espagne']='es'
#iso['brésil']='br'
#iso['france']='fr'
...
class document:
    ...
    def pos_tagging(self):
        ...
        if self.lang in ('brésil','chine','pologne','russie','france','italie','espagne'):
            ...
            if result:
                ...
                for l2 in lines[1:-2]:
                    ...
                    if l[0] in ('*','+') and re.search(r"-",s):
                        ...
                        return (True, u'dico composé - '+ l.decode('utf-8'))
                    if l[0] in ('*','+'):
                        log.info("Simple word : " + l + "\n")
                        return (True, u'dico simple - '+ l.decode('utf-8'))
                    elif l[0] in ('#'):
                        log.info("Unknwon word, no suggestion : " + unicode(l) + "\n")
...
def add_neologisms_to_db(neolist,lang):
    ...
    #proc_name = 'ADD_NEOLOGISM_GLOBAL' 
    try:
        conn = mysql.connector.connect(host='localhost',
                                       database='datatables',
                                       user='root',
        ...
        if conn.is_connected():
            for neo in neolist:
                ...
                log.info("Adding neologism to CN : ",args)
                cursor = conn.cursor()
...
if __name__ == '__main__':
    ...
    logging.basicConfig(format=FORMAT, datefmt='%m/%d/%Y %I:%M:%S %p', filename="./log/detect_neologisms_es.py.log", filemode='w', level=logging.INFO)    
