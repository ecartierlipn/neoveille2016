...
$('body').on('change',"#currentLanguage",function(){
                val = this.value;
                console.log(val);
                $.blockUI({ css: { 
            		border: 'none', 
...
$('#exampleNeo thead tr:eq(1) th').each( function () {
	title = $(this).text();
	if (title.length > 0){
        $(this).html( '<input type="text" class="column_search form-control" style="width:1O%" placeholder="'+title+'"  />' ); // form-control-sm"  
	}
	else{
        $(this).html( '&nbsp;' );  	
	}
 	...
 	if (editorNeo.lang == undefined){editorNeo.lang='fr';}
        if ( row.child.isShown() ) {
            ...
            $.blockUI({ css: { 
            		...
            		opacity: .5, 
            		color: '#fff',
            		message:'<h2>Veuillez patienter...</h2>'
        		} });
