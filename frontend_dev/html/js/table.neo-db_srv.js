...
$('body').on('change',"#langA",function(){
                val = this.value;
                console.log(val);
                $('#exampleNeo').DataTable().ajax.url("php/table.neo-db_srv.php?lang="+val).load();
                editorNeo.ajax = "php/table.neo-db_srv.php?lang="+val;
                editorNeo.s.ajax = "php/table.neo-db_srv.php?lang="+val;
                editorNeo.lang = val;
            });
...
editorNeo = new $.fn.dataTable.Editor( {
		ajax: {url:"php/table.neo-db_srv.php?lang="+languageW,type:"POST"},
...
// Setup filter- add a text input to each header cell
$('#exampleNeo thead tr:eq(1) th').each( function () {
	title = $(this).text();
	if (title.length > 0){
        $(this).html( '<input type="text" class="column_search form-control" style="width:100% !important;font-size: 0.90em;" placeholder="'+title+'"  />' ); // form-control-sm"  
	}
	else{
        $(this).html( '&nbsp;' );  	
	}
...
//        		fl:'contents,ttf(contents,"'+ d.termes_copy.terme + '")',
	    ...
	    console.log(num);
            ...
            restable = '<table width="100%" style="width:100% !important;font-size: 0.95em;">' + thead + tbody + '</table>';
            callback(restable);
    	},
        error: function (request) {
            alert("Error : " + request.status + ". Response : " +  request.statusText);
